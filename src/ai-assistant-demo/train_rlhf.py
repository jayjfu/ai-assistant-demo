...
parser.add_argument('--sft_model', default="./checkpoints/Llama-3.2-1B-fine-tuned-model/checkpoint-6928", type=str)
parser.add_argument('--reward_model', default="./checkpoints/Llama-3.2-1B-reward-model/checkpoint-40200", type=str)
...
parser.add_argument('--kl_coef', default=0.05, type=float)
...
def train(args):
    ...
    sft_model_path = os.path.join(str(SCRIPT_DIR), args.sft_model)
    reward_model_path = os.path.join(str(SCRIPT_DIR), args.reward_model)

    tokenizer = AutoTokenizer.from_pretrained(sft_model_path)

    model = AutoModelForCausalLM.from_pretrained(sft_model_path, dtype=torch.bfloat16)
    value_head = nn.Linear(model.config.hidden_size, 1, dtype=torch.bfloat16)
    ref_model = AutoModelForCausalLM.from_pretrained(sft_model_path, dtype=torch.bfloat16)
    reward_model = AutoModelForSequenceClassification.from_pretrained(reward_model_path, num_labels=1, dtype=torch.bfloat16)
